...
def test_lan_discovery_returns_ip_when_device_responds(monkeypatch):
    ...
    ip = setup_mod.lan_discover(target_sku="H6006", target_device="DE:AD:BE:EF:CA:FE:00:01",
...
def test_lan_discovery_returns_none_when_no_match(monkeypatch):
    ...
    assert setup_mod.lan_discover("H6006", "DE:AD", "10.0.0.58", timeout=0.1) is None
...
def test_validate_cloud_passes_when_device_present():
    ...
    assert setup_mod.validate_cloud("k", "H6006", "DE:AD:BE:EF:CA:FE:00:01", http=http) is True
...
def test_validate_cloud_returns_false_when_missing():
    ...
    assert setup_mod.validate_cloud("k", "H6006", "DE:AD", http=http) is False
...
def test_validate_cloud_raises_on_auth_failure():
    ...
    with pytest.raises(setup_mod.SetupError):
        setup_mod.validate_cloud("k", "H6006", "DE:AD", http=http)
